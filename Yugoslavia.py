...
with open('Olympics-Winter.csv') as csvfile:
	...
	for row in reader:
		if line_count is 0:
			...
			yugoslavia.append([int(row[0]), row[5], row[6], row[7]]) # multidimensional array
		else:
			world.append([int(row[0]), row[5], row[6], row[7]])
		line_count += 1

print('total medals for Yugoslavia:', len(yugoslavia))
...
for medal in yugoslavia:
	if medal[0] == 1992 and medal[3] == "Gold":
					gold_1992.append(medal)


print('yugoslavia won', len(gold_1924), 'gold medals in 1924')
print('yugoslavia won', len(gold_2014), 'gold medals in 2014')
python3 Luxembourg.py
